...
export function RoutineCard({ routine, onStart, onUpdate }: RoutineCardProps) {
	const [isExpanded, setIsExpanded] = useState(false);

	return (
		<div className="rounded-2xl bg-white shadow-md hover:shadow-lg transition-all duration-300 overflow-hidden border border-slate-200">
			<div className="w-full">
				<div className="p-5 md:p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
					<button
						className="w-full text-left flex items-start justify-between gap-4 hover:bg-slate-50 -m-5 md:-m-6 p-5 md:p-6 transition-all duration-200 group"
					...
					>
						...
						<div
							className={`text-slate-400 group-hover:text-slate-600 transition-transform duration-300 shrink-0 ${
					...
					<button
						className="w-full md:w-auto px-5 py-2.5 rounded-xl bg-gradient-to-r from-blue-500 to-blue-600 text-white hover:from-blue-600 hover:to-blue-700 font-semibold shadow-md hover:shadow-lg active:scale-[0.97] transition-all duration-200 flex items-center justify-center gap-2 shrink-0"
						onClick={onStart}
					>
						<svg
							className="w-5 h-5"
